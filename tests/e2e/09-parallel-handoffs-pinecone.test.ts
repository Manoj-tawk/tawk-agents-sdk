 ...
 * - maxTurns set to 12 for complex multi-agent workflows
...
async function agenticRAG(query: string): Promise<AgenticRAGResult> {
  ...
  // Increased maxTurns to 12 to allow for: triage routing + tool calls + handoff + specialist processing
  const result = await run(triageAgent, query, { maxTurns: 12 });
