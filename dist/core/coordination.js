...
exports.runWithJudge = runWithJudge;
 ...
 * ```
 */
async function raceAgents(agents, input, options = {}) {
    if (agents.length === 0) {
        throw new Error('No agents provided for race');
    }
    if (agents.length === 1) {
        const result = await (0, runner_1.run)(agents[0], input, options);
        return {
            ...result,
    ...
    const promises = agents.map(async (agent) => {
        try {
            const result = await (0, runner_1.run)(agent, input, options);
...
async function runParallel(agents, input, options = {}) {
    ...
    const inputs = Array.isArray(input) && agents.length > 1
        ? input
        : agents.map(() => input);
    if (inputs.length !== agents.length) {
        throw new Error(`Input count (${inputs.length}) must match agent count (${agents.length})`);
    }
    // Execute all agents in parallel
    const promises = agents.map(async (agent, index) => {
        try {
            const result = await (0, runner_1.run)(agent, inputs[index], options);
 ...
 * ```
 */
async function runWithJudge(workerAgents, judgeAgent, input, options = {}) {
...
${workerResult.results
        .map((r, i) => `[${workerAgents[i]?.name || `Agent ${i + 1}`}]:
...
Please evaluate these outputs and select or synthesize the best response.`;
    // Run judge agent
    const judgeResult = await (0, runner_1.run)(judgeAgent, judgeInput, options);
    return {
        ...judgeResult,
 ...
 * ```
 */
async function runHierarchical(coordinatorAgent, input, options = {}) {
    ...
    return await (0, runner_1.run)(coordinatorAgent, input, options);
}
