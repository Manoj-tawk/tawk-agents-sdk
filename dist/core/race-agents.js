...
async function raceAgents(agents, input, options = {}) {
    ...
    if (agents.length === 1) {
        const result = await (0, agent_1.run)(agents[0], input, options);
        return {
            ...result,
    ...
    const promises = agents.map(async (agent, index) => {
        try {
            const result = await (0, agent_1.run)(agent, input, {
                ...options,
    ...
    try {
        ...
        const allResults = await Promise.race([
            ...
            options.timeoutMs ? new Promise((_, reject) => setTimeout(() => reject(new Error(`Race timeout after ${options.timeoutMs}ms`)), options.timeoutMs)) : Promise.resolve(null)
        ]);
        if (!allResults) {
...
//# sourceMappingURL=race-agents.js.map